extends Control
## Pure display upgrade button - receives all data, only calls back for purchases
##
...
#region Pure Display - No Manager Calls
...
func display(data: UpgradeDisplayData) -> void:
	...
	# Apply all display data directly
...
#region Purchase System - Single Callback
...
# Complete the purchase - ONLY callback to manager
func _complete_purchase() -> void:
	hold_timer = 0.0
	_set_progress_percent(0.0)
	
	# Single manager call - everything else is handled by controller refresh
...
func _show_card_tooltip() -> void:
	...
	if card and ValidationUtils.has_event_bus():
		# Use event bus wrapper function for tooltips
