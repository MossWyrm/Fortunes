...
func get_description(card:Card, _bb_formatted = false) -> Dictionary:
	...
	if card.card_id_num < 500 && card.card_id_num % 100 < 11:
		...
		match card.card_id_num:
			...
			313:
				card_output += ("Flips your next %s "%[Stats.pent_queen_uses]+
			...
			314:
				card_output += (good_color+"Gives you +%s Protection and +%s charges"%[Stats.pent_king_value, Stats.pent_king_uses]+end_color_tag+
							" / "+
			...
			502:
				card_output +=("Pick a suit. %s random cards from this suit are "%[Stats.major_magician]+
							good_color+"added"+end_color_tag+
							" / "+
			...
			504:
				card_output +=("Remembers the last %s card(s) drawn and then "%[Stats.major_empress]+
			...
			506:
				card_output +=("Currently Unavailable")
			507:
				card_output += (good_color+"Copy %s cards "%[Stats.major_lovers]+end_color_tag+
							" / "+
			...
			518:
				card_output += ("Adds %s to the base value of all positive cards. Can be affected by The Moon."%[Stats.major_star]+
							" Lasts until your next shuffle")
			519:
				card_output += (good_color+"Multiply star value by %s"%[Stats.major_moon]+end_color_tag+
							" / "+
							bad_color+"Stars now work on negative cards"+end_color_tag+
							" until your next shuffle"
							)
			520:
				card_output += (good_color+"Add %s of The Star and %s of The Moon to your deck"%[Stats.major_sun_star, Stats.major_sun_moon]+end_color_tag+
							" / "+
			...
			521:
				card_output += ("Judgement "+good_color+"multiplies"+end_color_tag+" / "+
							...
							" the value of every card by %s until your next shuffle."%[Stats.major_judgement])
			522:
