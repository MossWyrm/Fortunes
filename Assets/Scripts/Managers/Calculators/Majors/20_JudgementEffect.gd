...
=== Judgement ===
When drawn:
- If upright (not flipped): adds 1 charge to Judgement, sets state to POSITIVE if charges > 0, else NEGATIVE/INACTIVE.
- If reversed (flipped): removes 1 charge from Judgement, sets state to POSITIVE if charges > 0, else NEGATIVE/INACTIVE.
...
func apply(card: Card, flipped: bool) -> int:
	if flipped:
		...
		powers_of_ten += 1
	state = DataStructures.CardState.POSITIVE if powers_of_ten > 0 else DataStructures.CardState.NEGATIVE if powers_of_ten <= 0 else DataStructures.CardState.INACTIVE
