...
- Sets state to POSITIVE if Moon is not NEGATIVE, else NEGATIVE.
...
func apply(card: Card, flipped: bool) -> int:
	# Use MajorCalculator's state and charge system for Star and Moon
	...
	star_value += game_state.stats.major_stats.star
