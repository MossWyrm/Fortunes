...
func set_game_state(state: GameState):
	if not ValidationUtils.is_valid_game_state(state):
		push_error("SaveManager: Invalid game state provided")
		return
	game_state = state
...
func _connect_events():
	SignalManager.safe_connect(event_bus.save_request, save_game, "SaveManager save_request")

func save_game():
	if not ValidationUtils.has_game_state():
		push_warning("SaveManager: Cannot save - no valid game state")
		return
